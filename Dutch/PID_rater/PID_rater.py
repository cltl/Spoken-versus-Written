...
from prep2 import prepare_data
...
print("Flickr30K written")
flickr_scores = []
for filename in ['Flickr30K-Val-tagged/val{}.out'.format(i) for i in range(5)]:
    output = Dutch_idea_density(filename, output='PID')
    flickr_scores.append(output)
    print(output)

print("DIDEC spoken")
didec_scores = []
for filename in ['./DIDEC-tagged/output_{}.txt'.format(i) for i in range(16)]:
    output = Dutch_idea_density(filename, output='PID')
    didec_scores.append(output)
    print(output)

print("Flickr average:", np.mean(flickr_scores))
print("DIDEC average:", np.mean(didec_scores))
